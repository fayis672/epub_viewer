...
function loadBook(data, opts) {
  ...
  // `data` is a base64 string on mobile (compact transfer over the JS bridge)
  // or a typed array / byte array on web. Normalize to a Uint8Array.
  var uint8Array;
  ...
  rendition.hooks.content.register((contents) => {
    ...
    var touchEndHandler = function (e) {
      ...
      // Word tap extraction
      try {
        var wordTapTouch = (e.changedTouches && e.changedTouches.length > 0) ? e.changedTouches[0] : (e.touches && e.touches.length > 0 ? e.touches[0] : null);
        var tapX = wordTapTouch ? wordTapTouch.clientX : currentTouchX;
        var tapY = wordTapTouch ? wordTapTouch.clientY : currentTouchY;
        ...
        if (tapX !== null && tapY !== null) {
          ...
          if (targetDoc.caretRangeFromPoint) {
            range = targetDoc.caretRangeFromPoint(tapX, tapY);
          } else if (targetDoc.caretPositionFromPoint) {
            var pos = targetDoc.caretPositionFromPoint(tapX, tapY);
            if (pos) {
          ...
          if (range && range.startContainer) {
            if (range.startContainer.nodeType === 3) {
              ...
              var child = range.startContainer.childNodes[range.startOffset] || range.startContainer.firstChild;
          ...
          if (!textNode && targetDoc.elementFromPoint) {
            var elem = targetDoc.elementFromPoint(tapX, tapY);
            if (elem) {
              for (var i = 0; i < elem.childNodes.length; i++) {
                if (elem.childNodes[i].nodeType === 3 && elem.childNodes[i].textContent.trim().length > 0) {
          ...
          if (textNode && textNode.textContent) {
            ...
            if (text.length > 0) {
              ...
              var word = text.slice(start, end).replace(/^[^\w]+|[^\w]+$/g, '');
              if (word.length > 0) {
                callFlutterHandler('wordTapped', word);
              }
            }
          }
        }
      } catch (wordErr) {
        // Ignore errors in word extraction
      }
