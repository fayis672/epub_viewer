...
function loadBook(data, cfi, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor) {
  ...
  rendition.hooks.content.register((contents) => {
    if (useCustomSwipe) {
      // Apply swipe detection to both document and body for better coverage
      const elements = [contents.document.documentElement, contents.document.body];
      
      elements.forEach(el => {
        if (el) {
          // Add a class to indicate this element has swipe detection
          el.classList.add('epub-swipe-enabled');
          
          // Apply the improved swipe detection
          detectSwipe(el, function (el, direction) {
            // Handle horizontal swipes for page navigation
            if (direction === 'l') {
              rendition.next();
            } else if (direction === 'r') {
              rendition.prev();
            ...
            // Optional: Handle vertical swipes for additional functionality
            // For example, you could implement chapter navigation with up/down swipes
            // if (direction === 'u') { /* previous chapter */ }
            // if (direction === 'd') { /* next chapter */ }
          });
        }
      });
      ...
      style.innerHTML = `
        ...
        /* Add a subtle touch effect to indicate the content is swipeable */
        .epub-swipe-enabled:active {
...
function detectSwipe(el, func) {
  // Improved swipe detection with better thresholds and handling
  let touchData = {
    startX: 0,
    startY: 0,
    endX: 0,
  ...
  // Configuration for better swipe detection
  const config = {
    minSwipeDistance: 30,       // Minimum distance for a swipe to be detected (pixels)
    maxSwipeTime: 300,          // Maximum time for a swipe to be valid (milliseconds)
    horizontalThreshold: 0.3,    // Horizontal movement must be this times more than vertical to be horizontal swipe
    verticalThreshold: 0.3,      // Vertical movement must be this times more than horizontal to be vertical swipe
    preventDefaultOnMove: false  // Only prevent default if we're sure it's a swipe
  };
  
  // Add visual feedback element
  const feedbackEl = document.createElement('div');
  feedbackEl.className = 'swipe-feedback';
  feedbackEl.style.cssText = 'position:fixed; top:50%; left:50%; transform:translate(-50%,-50%); ' +
                           'background:rgba(0,0,0,0.2); color:white; border-radius:50%; ' +
                           'width:80px; height:80px; display:none; justify-content:center; ' +
                           'align-items:center; font-size:24px; z-index:9999; opacity:0; ' +
  ...
  function showFeedback(direction) {
    let icon = '→';
    if (direction === 'l') icon = '←';
    else if (direction === 'u') icon = '↑';
    else if (direction === 'd') icon = '↓';
    
    feedbackEl.innerHTML = icon;
  ...
  el.addEventListener('touchstart', function(e) {
    ...
    touchData.isSwipeInProgress = true;
  }, { passive: true });
  
  // Touch move event
  el.addEventListener('touchmove', function(e) {
    if (!touchData.isSwipeInProgress) return;
    ...
    const deltaX = Math.abs(touchData.endX - touchData.startX);
    const deltaY = Math.abs(touchData.endY - touchData.startY);
    
    // Only prevent default if it looks like a deliberate swipe
    if (config.preventDefaultOnMove && 
        ((deltaX > config.minSwipeDistance && deltaX > deltaY * config.horizontalThreshold) || 
         (deltaY > config.minSwipeDistance && deltaY > deltaX * config.verticalThreshold))) {
      e.preventDefault();
    }
  }, { passive: !config.preventDefaultOnMove });
  
  // Touch end event
  el.addEventListener('touchend', function(e) {
    if (!touchData.isSwipeInProgress) return;
    touchData.isSwipeInProgress = false;
    ...
    if (swipeTime < config.maxSwipeTime && (absX > config.minSwipeDistance || absY > config.minSwipeDistance)) {
      let direction = '';
      
      // Determine if horizontal or vertical swipe based on which had more movement
      if (absX > absY * config.horizontalThreshold) {
        // Horizontal swipe
        direction = deltaX > 0 ? 'r' : 'l';
      } else if (absY > absX * config.verticalThreshold) {
        // Vertical swipe
...
function saveReadingState() {
  try {
    ...
    let state = {
      bookId: bookId,
      cfi: rendition && rendition.location ? rendition.location.start.cfi : null,
 ...
 * @param {string} type - Type of data to clear: 'all', 'bookmarks', 'highlights', or 'current-book'
...
function clearStorage(type) {
  try {
    if (type === 'all') {
      ...
      highlights = [];
      // Clear highlight annotations from the current view
      if (rendition) {
        rendition.annotations.remove('hl');
      }
      saveReadingState();
    } else if (type === 'current-book') {
      // Clear data only for the current book
      // Get current book identifier
      let currentBookId = null;
      if (book && book.key) {
        currentBookId = book.key;
      } else if (book && book.package && book.package.metadata) {
        const metadata = book.package.metadata;
        const idComponents = [
          metadata.title,
        ...
        currentBookId = idComponents || 'unknown-book';
      }

      // If we can identify the current book, clear only its data
      if (currentBookId) {
        const stateJson = localStorage.getItem('epubReaderState');
        if (stateJson) {
          const state = JSON.parse(stateJson);
          // Keep the state but clear book-specific data
          state.bookmarks = [];
          state.highlights = [];
          state.cfi = null;
          localStorage.setItem('epubReaderState', JSON.stringify(state));
    ...
    // Notify Flutter
    window.flutter_inappwebview.callHandler('storageCleared', type);
    return true;
  } catch (e) {
...
function loadReadingState() {
  try {
    ...
    if (stateJson) {
      ...
      // Check if saved state is for the current book
      const isSameBook = !state.bookId || state.bookId === currentBookId;

      // Always restore reader preferences regardless of book
      // Restore font size
      if (state.fontSize) {
      ...
      // Only restore book-specific state if it's the same book
      if (isSameBook) {
        // Restore bookmarks
        ...
        // Restore position
        if (state.cfi) {
          rendition.display(state.cfi);
        }
      } else {
        // Different book, reset book-specific state
        bookmarks = [];
        highlights = [];
      }
    }
  } catch (e) {
    console.error("Error loading reading state:", e);
  }
}